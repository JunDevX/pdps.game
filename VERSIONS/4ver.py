...
import requests
import zipfile
import shutil  # Добавлен для удаления папки с игрой

# --- ОПРЕДЕЛЕНИЕ ПУТИ К ИКОНКЕ (С поддержкой PyInstaller) ---
...
# Резервный путь, если в контейнере _MEIPASS иконки нет (лежит рядом с exe)
...
if sys.platform == "win32":
    import ctypes
    # Обновлен ID для версии 1.1.2
    myappid = "zapl.pixeldash.launcher.1.1.2"  
...
CURRENT_VERSION = "1.1.2" # Обновлена версия
...
DROPBOX_ZIP_URL = "https://www.dropbox.com/scl/fi/h7405cnmwuteji0hl55kj/Pixel-Dash.zip?rlkey=btnbnzrohe3f05psf57jxumvd&st=j43ipqia&dl=1"
...
class LoadingWorker(QObject):
    ...
    def parse_version(self, ver_str):
        try:
            return list(map(int, ver_str.replace("v", "").replace("V", "").split('.')))
        except Exception:
            return [0, 0, 0]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if latest_version != CURRENT_VERSION:
                    ...
                    # Если версия на GitHub строго новее текущей
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Определение путей к SVG
        self.resources_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "resources")
        self.trash_svg_path = os.path.join(self.resources_dir, "trash_icon.svg")
        self.reboot_svg_path = os.path.join(self.resources_dir, "reboot_icon.svg")
        
        # Генерация SVG файлов в папке resources, если их нет
        ...
        self.main_style = """
            ...
            QPushButton {
                ...
                border: none;
                padding: 10px 24px;
                border-radius: 22px; /* Изменено на 22px для идеальной закругленности */
                font-size: 14px;
                font-weight: 900;
                font-family: 'Segoe UI', Arial, sans-serif;
            }
    ...
    def create_svg_icons(self):
        # Создаем папку resources, если ее еще нет
        if not os.path.exists(self.resources_dir):
            os.makedirs(self.resources_dir)

        trash_svg = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M16 9v10H8V9h8m-1.5-6h-5l-1 1H5v2h14V4h-3.5l-1-1zM18 7H6v12c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7z"/></svg>"""
        reboot_svg = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>"""
        
        if not os.path.exists(self.trash_svg_path):
            with open(self.trash_svg_path, "w", encoding="utf-8") as f: 
                f.write(trash_svg)
        if not os.path.exists(self.reboot_svg_path):
            with open(self.reboot_svg_path, "w", encoding="utf-8") as f: 
                f.write(reboot_svg)
    ...
    def init_ui(self):
        ...
        # -- КРУГЛЫЕ КНОПКИ SVG С ПОДДЕРЖКОЙ СЕРОГО ЦВЕТА (disabled) --
        action_btns_layout = QHBoxLayout()
        action_btns_layout.setAlignment(Qt.AlignmentFlag.AlignLeft)
        action_btns_layout.setSpacing(12)

        self.btn_delete = QPushButton()
        self.btn_delete.setFixedSize(36, 36)
        self.btn_delete.setIcon(QIcon(self.trash_svg_path))
        self.btn_delete.setToolTip("Удалить игру")
        self.btn_delete.setStyleSheet("""
            QPushButton { background-color: rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 6px; }
            QPushButton:hover { background-color: #E53935; }
            QPushButton:disabled { background-color: rgba(255, 255, 255, 0.05); opacity: 0.4; }
        """)
        self.btn_delete.clicked.connect(self.delete_game)

        self.btn_reinstall = QPushButton()
        self.btn_reinstall.setFixedSize(36, 36)
        self.btn_reinstall.setIcon(QIcon(self.reboot_svg_path))
        self.btn_reinstall.setToolTip("Переустановить игру")
        self.btn_reinstall.setStyleSheet("""
            QPushButton { background-color: rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 6px; }
            QPushButton:hover { background-color: #FF9800; }
            QPushButton:disabled { background-color: rgba(255, 255, 255, 0.05); opacity: 0.4; }
        """)
        self.btn_reinstall.clicked.connect(self.reinstall_game)

        action_btns_layout.addWidget(self.btn_delete)
        action_btns_layout.addWidget(self.btn_reinstall)
        # ------------------------------------
        ...
        left_layout.addSpacing(10)
        left_layout.addLayout(action_btns_layout)
        ...
        self.ch_text.setHtml("<span style='color: rgba(255, 255, 255, 0.6);'>Синхронизация патчноутов...</span>")
        ...
        discord_btn.setStyleSheet("""
            QPushButton { background-color: #5865F2; font-size: 15px; font-weight: 800; border-radius: 24px; }
        ...
        skip_btn.setStyleSheet("""
            QPushButton { background: transparent; color: rgba(255,255,255,0.5); font-size: 13px; font-weight: 700; border: 1px solid rgba(255,255,255,0.2); border-radius: 16px; }
    ...
    # --- ОБНОВЛЕННЫЕ ФУНКЦИИ ДЛЯ КНОПОК ---
    def delete_game(self):
        game_path = os.path.join(APPDATA_DIR, "Pixel Dash")
        ...
        self.install_btn.setStyleSheet("background-color: #2196F3; font-weight: 900; border-radius: 22px;")
        try: self.install_btn.clicked.disconnect()
        except TypeError: pass
        self.install_btn.clicked.connect(self.start_installation)
        
        # После полного удаления деактивируем кнопки
    ...
    def on_loading_finished(self, data):
        ...
        # Прямая отрисовка полученного Markdown
        self.ch_text.setMarkdown(data["changelog"])
        self.app_data_status = data
        
        self.stacked_widget.setCurrentWidget(self.main_page)

        # Проверяем, установлена ли игра, и включаем/выключаем кнопки
        is_installed = data["game_installed"]
        self.btn_delete.setEnabled(is_installed)
        self.btn_reinstall.setEnabled(is_installed)

        if data["update_type"] == "fix":
            self.install_btn.setText("Обновить")
            self.install_btn.setStyleSheet("background-color: #FFEB3B; color: #000000; font-weight: 900; border-radius: 22px;")
            try: self.install_btn.clicked.disconnect()
            except TypeError: pass
            self.install_btn.clicked.connect(lambda: QDesktopServices.openUrl(QUrl(data.get("html_url", "https://github.com/JunDevX/pdps.game/releases"))))
        else:
            if is_installed:
    ...
    def start_installation(self):
        ...
        # На время скачивания/установки выключаем доп. кнопки
        self.btn_delete.setEnabled(False)
        self.btn_reinstall.setEnabled(False)
    ...
    def on_installation_finished(self, success, message):
        ...
        if success:
            ...
            # Включаем кнопки после успешной установки
            self.btn_delete.setEnabled(True)
            self.btn_reinstall.setEnabled(True)
        else:
            self.ch_text.setMarkdown(message)
            # Если произошла ошибка, возвращаем состояние кнопок обратно, проверив файл
            game_exe_path = os.path.join(APPDATA_DIR, "Pixel Dash", "GeometryDash.exe")
            has_game = os.path.exists(game_exe_path)
            self.btn_delete.setEnabled(has_game)
            self.btn_reinstall.setEnabled(has_game)
    ...
    def set_btn_to_play(self):
        ...
        self.install_btn.setStyleSheet("background-color: #4CAF50; font-weight: 900; color: white; border-radius: 22px;")
        try: self.install_btn.clicked.disconnect()
        except TypeError: pass
        self.install_btn.clicked.connect(self.start_installation)
...
if __name__ == "__main__":
    ...
    # Безопасная установка иконки для QApplication
    if os.path.exists(ICON_PATH):
